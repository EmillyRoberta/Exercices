...
export function monthRate(ratePerHour, discount) {
  ...
  var tudo = ((ratePerHour) * 8) * 22;
 
  var desconto = tudo*discount;

  return Math.ceil(tudo - desconto);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  ...
  var diaDepois = dayRate(ratePerHour) - dayRate(ratePerHour) * discount;

  var orcamento = budget/diaDepois;
